...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of cores available/ to use.",
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
...
def main():
    ...
    if not latent_files:
        raise ValueError(f"No .npy files found in {latent_folder}")
    ...
    if not args.finetune:  # Needs some checking
        ...
        )  # Replace the StepLR scheduler with the CosineAnnealingLR scheduler
        start_epoch = resume_from_checkpoint(
            checkpoint_path, model, optimizer, scheduler
        )
    ...
    for i in range(start_epoch, args.max_epochs):
        ...
        for step, batch in enumerate(train_data_loader):
            ...
            loss.backward()  # Accumulate gradients every step
            train_loss += loss.item() * accumulation_steps  # Track unscaled loss
